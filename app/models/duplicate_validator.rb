class DuplicateValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    ...
    if record.id.present?
      not_own_periods = Event.where('id NOT IN (?) AND start_date < ? AND end_date > ?', record.id, new_end_date, new_start_date)
    else
      not_own_periods = Event.where('start_date < ? AND end_date > ?', new_end_date, new_start_date)
    end
